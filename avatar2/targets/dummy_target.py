...
class DummyTarget(Target):
    ...
    def __init__(self, name, avatar):
        super(DummyTarget, self).__init__(name, avatar)
        # List of breakpoints
        self.bp      = []
    ...
    def init(self):
        ...
        for range in self.avatar.memoryRanges:
            range = range.data
            if range.forwarded == True:
                self.mranges.append((range.address, range.address+range.size))
        self.wait()
    ...
    def set_breakpoint(self, line, hardware=False, temporary=False, regex=False, condition=None, ignore_count=0, thread=0):
        self.bp.append(line)
