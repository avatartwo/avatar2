...
class ARMv7MInterruptRecordingProtocol(Thread):
    ...
    def inject_monitor_stub(self, addr=0x20010000, vtor=0x20011000, num_isr=48):
        ...
        self.target.inject_asm(self._get_stub(), self._monitor_stub_base, patch={stub_offset: b'\xef\xf3\x05\x80'})
        self.target.write_memory(self._monitor_stub_isr - 4, size=4, value=0x00)  # set irq_buffer_ptr to 0
        self.target.write_memory(self._monitor_stub_mtb, size=1, value=0xff)  # Ensure end of buffer flag
        ...
        self.target.write_memory(self._monitor_stub_vt_buffer, value=self.original_vt, size=4, num_words=num_isr)
    ...
    def run(self):
        ...
        try:
            while not (self.avatar._close.is_set() or self._close.is_set()):
                curr_isr = self.target.read_memory(address=self._monitor_stub_trace_buffer + buffer_pos, size=1)
                if curr_isr == 0xff:
                    sleep(TICK_DELAY)
                    continue
                ...
                if curr_isr > 0x80:
                    ...
                    self._dispatch_message(
                        TargetInterruptExitMessage(self.target, self.msg_counter, interrupt_num=curr_isr,
                                                   isr_addr=addr))
                else:
                    addr = self.original_vt[curr_isr]
                    self._dispatch_message(
                        TargetInterruptEnterMessage(self.target, self.msg_counter, interrupt_num=curr_isr,
                                                    isr_addr=addr))
